...
# Fit a Ridge estimator with cross-validation and find the optimal value of alpha
est_list = [Ridge(alpha=0.1,fit_intercept=False).fit(X, y)  for y in y_list]

def coef_pval(e, x, y):
    tvals = e.coef_ / np.sqrt(e.alpha)
    pvals = 2 * (1 - stats.t.cdf(np.abs(tvals), len(x) - len(e.coef_)))
    return pvals
# Get coefficient values and p-values
coef_list = [est.coef_ for est in est_list]
pval_list = [(coef_pval(est, X, y))for est, y in zip(est_list, y_list)]
