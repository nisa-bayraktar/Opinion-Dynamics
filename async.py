        ...
        for t in range (0,100): 
            ...
            for node in nodes:
                ...
                if len(neighbours)>0: # what happens ıf node has no neıghbours (all weıghts ınto node are equal to zero)?
                    ...
                    print(G[j][node]["weight"]for j in neighbours)
                    sum2 = sum(G[j][node]["weight"]for j in neighbours)
                    if sum2>0:
                        avg = sum(G.nodes[j]["opinion_state"] * G[j][node]["weight"] for j in neighbours) / sum2
                    ...
                    for j in neighbours:
                        ...
                        G[j][node]["weight"] += homophily * (t_h - dif)  * dt
                        if sum2>0:
                            dif = abs(avg - G.nodes[j]["opinion_state"]) 
                            G[j][node]["weight"] += novelty * (dif - t_a) * dt
